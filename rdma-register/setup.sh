...
# 部署nic
kubectl apply -f ./rdma-register/recipes/nicclusterpolicy.yaml
...
kubectl apply -f ./rdma-register/recipes/macvlan-ippools.yaml
kubectl apply -f ./rdma-register/recipes/macvlan-networks.yaml
...
# 测试单卡路由
kubectl apply -f ./rdma-register/examples/test-single.yaml
...
ib_write_bw -d mlx5_10 -q 1 --report_gbits -F -a -x 5 10.244/

# 测试聚合路由
kubectl apply -f ./rdma-register/examples/test-agg.yaml
kubectl get pods -n dynamo-system -o wide | grep rdma
...
# 清理测试pod
kubectl delete -f ./rdma-register/examples/test-single.yaml --ignore-not-found=true
kubectl delete -f ./rdma-register/examples/test-agg.yaml --ignore-not-found=true
...
#（可选）卸载
kubectl delete -f ./rdma-register/recipes/macvlan-networks.yaml --ignore-not-found=true
kubectl delete -f ./rdma-register/recipes/macvlan-ippools.yaml --ignore-not-found=true

kubectl get network-attachment-definitions -A || true

kubectl delete -f ./rdma-register/recipes/nicclusterpolicy.yaml --ignore-not-found=true
kubectl delete nicclusterpolicies.mellanox.com nic-cluster-policy --ignore-not-found=true
kubectl -n nvidia-network-operator get pods

# 可选(multus无法跨命名空间权限隔离，按需删除)
