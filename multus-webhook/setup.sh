...
# 1.1 制作镜像
docker build -t webhook:v6 -f Dockerfile .
...
kubectl -n dynamo-system get pod webhook-test -o yaml 

# 2.8 卸载
kubectl delete pod webhook-test-06 -n dynamo-system --ignore-not-found=true
kubectl delete pod webhook-test-14 -n dynamo-system --ignore-not-found=true
kubectl delete -f test.yaml --ignore-not-found=true || true

kubectl delete -f multus_webhook_mwc.yaml --ignore-not-found=true || true
kubectl delete mutatingwebhookconfiguration multus-networks-injector --ignore-not-found=true

kubectl delete -f multus_webhook_deploy.yaml --ignore-not-found=true || true
